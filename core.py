...
def DarkLight(halo, DMO=True, vmax_file=None, n=1, 
    ...
    if mergers and sum(occupied)>0:
        zmerge, qmerge, hmerge, msmerge = accreted_stars(halo,vthres=vthres,zq=zq,timesteps=timesteps,occupation=occupation,DMO=DMO,
    ...
    return tt,zz,vsmooth_cored,sfhs_insitu,mstars_insitu,mstars_tot
...
def accreted_stars(halo, vthres=26.3, zq=4., plot_mergers=False, verbose=False, n=1,
                   prequench='fiducial',postquench='schechter',postquench_scatter='increasing',
                   mapping='3bins', timesteps='sim',occupation=2.5e7, DMO=False):
    """
    ...
    for ii,im in enumerate(range(len(zmerge))):
        ...
        for hsub in hmerge[im][1:]:
            ...
            t_sub,z_sub,vsmooth_cored_sub,sfh_sub,mstar_insitu_sub,mstar_tot_sub = DarkLight(hsub, n=n, vthres=vthres, zq=zq,
